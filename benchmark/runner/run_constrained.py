...
def emulate_network_properties(netem_options):
    ...
    try:
        subprocess.check_call(
            qdisc_cmds + ["add"] + dev_cmds + ["netem"] + netem_options
        )
        yield
    finally:
        try:
            ...
            raise RuntimeError(
                "Could not reset 'tc' properties! You may have to do so manually with 'sudo tc qdisc del dev lo root'."
            ) from exc
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Running constrained measurement with options '{' '.join(netem_options)}'."
        )
        with open(out_path, mode="wb") as out_f:
            with emulate_network_properties(netem_options):
                subprocess.check_call(
                    [client_exe] + cmdline_args, stdout=out_f, env=env_no_threading
                )
    finally:
